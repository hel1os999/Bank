...
from core.models import UserDataBase, BankBase
...
db_config = {
    "engine1": {"url": str(settings.db.userdata_url)},
    "engine2": {"url": str(settings.db.bank_url)},
}
...
# }
target_metadata = {
    'engine1':UserDataBase.metadata,
    'engine2':BankBase.metadata
}
...
async def run_migrations_offline() -> None:
    ...
    for name, rec in db_config.items():
...
async def run_migrations_online() -> None:
    engines = {
        name: create_async_engine(cfg["url"], poolclass=NullPool)
